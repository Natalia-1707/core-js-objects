...
function removeProperties(obj, keys) {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    if (Object.keys(obj2).includes(keys[i])) {
      delete obj2[keys[i]];
    }
  }
  return obj2;
...
function compareObjects(obj1, obj2) {
  ...
  for (let i = 0; i < obj1Arr.length; i += 1) {
...
function makeWord(lettersObject) {
  const result = [];
  const resultLength = Object.values(lettersObject).flat().length;
  for (let j = 0; j < resultLength; j += 1) {
    result.push('');
  }
  const array = Object.entries(lettersObject);
  for (let i = 0; i < array.length; i += 1) {
    for (let j = 0; j < array[i][1].length; j += 1) {
      result.splice(array[i][1][j], 1, array[i][0]);
    }
  }
  return result.join('');
}
